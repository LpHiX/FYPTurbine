"""
Goldman (1970) NASA TM X-2095 — exact replication of Figs 4, 5, 8, 9.
...
the Sasman-Cresci (1966) compressible turbulent integral BL solver
(the method Goldman's TM X-2095 analysis is built on).
...
# ══════════════════════════════════════════════════════════════════════
# Boundary layer solver (Sasman-Cresci 1966, reference temperature)
# ══════════════════════════════════════════════════════════════════════
...
def main():
    ...
    ax5.plot(s_lo_sc, Me_lo_sc, 'b-', lw=2, label='Lower surface')
    ax5.plot(s_up_sc, Me_up_sc, 'r-', lw=2, label='Upper surface')
    ...
    for nu_l_deg in nu_l_range_deg:
        ...
        try:
            ...
            print(f"  nu_l={nu_l_deg:.0f}: M_l={Ml:.3f}, Hi_max(SC)={hmax_sc:.3f}")
    ...
    for nu_u_deg in nu_u_range_deg:
        ...
        try:
            ...
            print(f"  nu_u={nu_u_deg:.1f}: M_u={Mu:.3f}, Hi_max(SC)={hmax_sc:.3f}")
        except Exception as e:
    ...
    fig.suptitle(
        'Goldman (1970) Replication --- MoC blade + Sasman-Cresci BL\n'
    ...
    # Fig 5
    ax = axes[0, 0]
    ax.plot(s_lo_sc, Me_lo_sc, 'b-', lw=2, label='Lower')
    ax.plot(s_up_sc, Me_up_sc, 'r-', lw=2, label='Upper')
