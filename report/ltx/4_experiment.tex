...
\begin{figure}[H]
    ...
    \caption{TODO} % Fig 4.2: labelled rig setup photo/diagram -- which part is which (Knoll 06-09: labelled diagram in body, build photos -> appendix)
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{placeholder.png}
    \caption{TODO} % Fig 4.1: P&ID -- the main rig figure (tank -> supply pump -> inlet throttle -> pump -> outlet throttle -> return; sensor map pt_pump/pt_in/pt_out/fm0)
    \label{fig:water-pid}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{placeholder.png}
    \caption{TODO} % Fig 4.3: DAQ block diagram (sensor -> conditioning -> MCP3208 -> ESP32 -> Pi5 -> HDF5/GUI)
    \label{fig:daq-block}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{placeholder.png}
    \caption{TODO} % Fig 4.4: KiCAD schematic -- MCP3208 + LT3042 + op-amp torque conditioning (export from KiCAD project)
...
% STUB -- the instrument table the Ch5 uncertainty preamble cites.
...
% Known: TM306 <0.1% of 5 Nm; PT = generic 1.2 MPa 0.5-4.5 V ratiometric [%FS TBC]; fm0 flowmeter [TBC]; d2/b2 as-printed [measure].

\begin{table}[H]
    \centering
    \caption{TODO} % every channel: quantity, sensor, range, output type, systematic uncertainty + source
...
% STUB -- motor/ESC: no speed loop, throttle-stepped, torque/current cap -> pump testing topped out ~7,500 rpm;
% shop air: pressure + droop -> coupled run settled ~5,400 rpm;
% Knoll framing: "these conditions weren't surveyed due to experimental limitations" -- capability statement, not apology;
% safety: 2-3 sentences ONLY (blast shield, personnel out of room, PILZ interlock); full safety case -> appendix.