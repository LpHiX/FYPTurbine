...
\begin{figure}[H]
    ...
    \caption{Shaft alignment procedure using the flat faces of the component mounts and precision square blocks as references.} % PLACEHOLDER IMAGE: photo or diagram
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{placeholder.png}
    \caption{P\&ID of the water circuit. The tank feeds the supply pump, then the inlet throttle valve, the experimental pump, the outlet throttle valve and the return line. Pressure is measured at the supply (pt\textsubscript{pump}), the pump inlet (pt\textsubscript{in}) and the pump outlet (pt\textsubscript{out}), and flow in the inlet line (fm0).} % PLACEHOLDER IMAGE: P&ID (Safety Plan P&IDs can be reused)
    \label{fig:water-pid}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{placeholder.png}
    \caption{Data acquisition architecture. Sensors and the DSP6001 analog outputs feed the MCP3208 ADC, read by an ESP32-S3 over SPI, streamed to the Raspberry Pi 5 over UART, logged to HDF5 and forwarded to the control room GUI over the network.} % PLACEHOLDER IMAGE: block diagram
    \label{fig:daq-block}
\end{figure}
